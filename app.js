...
max_id = dataFull.pagination.next_max_id;// Глобальная переменная с индексом фото следующим за выборкой
...
//var date = new Date(1524766958 * 1000);
   ...
   description.innerHTML = descText +"<br/><br/><img name='heart' src='image/heart.png' width='20px' /> "
       + descLike + "  <img name='heart' src='image/bubble.png' width='20px' /> " + descComm +  "<br/>" + getCreatedTime(crTime);
...
function loadPhoto()
{
    ...
    window.max_id = nextPhotos.pagination.next_max_id;
    if (nextPhotos.data.length>0)
    {
        var content = doc.getElementById('content');
       // var b = doc.getElementById('button');
...
function getCreatedTime(crTime) {
    var ret = "";
    var date = new Date(parseInt(crTime) * 1000);
